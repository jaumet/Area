...
echo "<div id ='debug'><hr />";
foreach ($fields as $n) {
	// get config for this field
	$f=$d['fields'][$n];

	if (!$f) {
		//echo "<div><p>".$f." ->note: field named $n is not defined in the config!</p></div>";
	}
	...
	//print_r($numdistinct);
	echo "<br />num distinct: ".$n.": ".$numdistinct;
	...
	echo "<br />num null: ".$n.": (".$numnull.")";
	

	$count++;
	$percdistinct = ($numdistinct*100)/($numrows - $numnull);
	$percnotnull  = (($numrows - $numnull)*100)/$numrows;

echo "<br />percdistinct: ".$percdistinct." %";

echo "<br />percnotnull: ".$percnotnull." %";
echo "<hr />";
