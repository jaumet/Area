...
if (!$dataname) {
  ...
  if (!$param1 OR !$param2) {
    ...
    foreach ($fields as $n) {
        ...
        $result = mysql_query($query) or die('Query count distinc at index.php: ' . mysql_error());
    ...
    #######################
		...
		$noparameters_html = "
			<div id=\"analisisdiv\">
			<br />
			<ul><li>About the data: <b>".$d['name']."</b></li>
			<li>Database Name: <b>".$d['db']['name']."</b></li>
			<li>Database Label: <b>".$d['label']."</b></li>
			<li>Table Name: <b>".$d['table']."</b></li>\n\n";

		# Numero de camps a la taula
		$noparameters_html .= "<li>Number of fields: <b>".$numfields."</b></li>\n\n";

		# Numero d'entrades
		$noparameters_html .=  "<li>Number of records: <b>".$numrows."</b></li>";
		$noparameters_html .=  "<li>Possible representations: <b>".pow(count($good),2)."</b></li></ul>";
  ...
  //Getting max num the nodes in a block
    ...
    $form_html .= '<h5 class="tab_title">Parameters</h5>';
    $form_html .= "<p>";
    ...
    <span class="fb_required">Blocks</span>'."\n".'
    <select class="fb_select" id="param1" name="param1">'."\n".'
      <option value="">-select-</option>'."\n".'
      '.$options1.'
    </select>'."\n".'
    <img src="./images/213px-Arrowlr.svg.png" width="35px" />&nbsp;&nbsp;&nbsp;&nbsp;<span class="fb_required">Colors</span>'."\n".'
    ...
    $form_html .= '</form>'."\n";
    $form_html .= '<h5 class="tab_title">Legend</h5>';
    $form_html .=  $legend;
    ### END FORM
    $form_html .= "</p></div>"."\n";
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    ...
    if ($param1 AND $param2) { // IF L325
        ...
        $blockstyle = "width: ".($block_x)."px; height:".($block_y + 130)."px;";
	    ...
	    $myblockname = t($block_array_h[$s], $language);
	    if (strlen($myblockname)>100) {
		$myblockname = substr($myblockname,0,95)." (...) ";
	    }	
            echo '<div class="block" style="'.$blockstyle.'">'."\n";
            echo '<div class="blockname">'.$myblockname.' <i>['.$percertage.'%] '.$block1_array[$bl].'</i></div>';
            $s++;
            if ($block1_array[$bl])  {
                if ($quantum != "quantum") {
	...
	if ($_POST['language'] == "en") {
        ...
        echo '<h2>About this visualization</h2>';
	echo '<h3><b>'.$d['label'].'</b></h3>';
	echo '<p>Num. of reviwed tools: <b>'.$total_nodes.'</b></p>';
	echo '<p>Representations: <b>'.count($d['fields']).'</b></p>';
